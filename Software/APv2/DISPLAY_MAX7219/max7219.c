...
void max7219_shiftout_HW(uint8_t bytedata) 
{
	SPDR = bytedata;                
	while(!(SPSR & (1<<SPIF)));
}
...
void max7219_send(uint8_t icnum, uint8_t reg, uint8_t data) {
	...
	if(icnum < MAX7219_ICNUMBER) {
		...
		for(i=icnum; i<(MAX7219_ICNUMBER-1); i++) {
			max7219_shiftout_HW(MAX7218_REGNOOP); //no op reg
			max7219_shiftout_HW(MAX7218_REGNOOP); //no op data
		}
		//send info to current ic
		max7219_shiftout_HW(reg); //send reg
		max7219_shiftout_HW(data); //send data
